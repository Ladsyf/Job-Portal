...
session_start();
$target_dir = "../resume/";
$path = $_FILES['editresume']['name'];
$ext = pathinfo($path, PATHINFO_EXTENSION);
// $target_file = $target_dir . basename($_FILES["fileToUpload"]["name"]);
$target_file = $target_dir . $_SESSION['id'].$_SESSION['acc_type'].$_SESSION['username']."-resume.".$ext;
$uploadOk = 1;
$imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));

// Check if image file is a actual image or fake image
// if(isset($_POST["submit"])) {
//   $check = getimagesize($_FILES["editresume"]["tmp_name"]);
//   if($check !== false) {
...
// Check file size
if ($_FILES["editresume"]["size"] > 5000000) {
...
// if everything is ok, try to upload file
  ...
  if (move_uploaded_file($_FILES["editresume"]["tmp_name"], $target_file)) {
    ...
    $newNode->appendChild($xml->createElement("resume", $_SESSION['id'].$_SESSION['acc_type'].$_SESSION['username']."-resume.".$ext));//
